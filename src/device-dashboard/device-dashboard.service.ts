...
export class DeviceDashboardService {
  ...
  private async loadDevice(deviceId: string) {
    ...
    if (redisClient) {
      try {
        ...
        this.logger.error(
          `[REDIS CACHE] Error reading from Redis: ${err.message}`,
        );
      }
    }
    ...
    if (!device) {
      ...
      try {
        ...
        this.logger.error(
          `[DATABASE] Failed loading device ${deviceId}: ${err.message}`,
        );
      ...
      if (device && redisClient) {
        try {
          this.logger.log(
            `[REDIS CACHE] Saving device ${deviceId} to Redis for ${this.CACHE_TTL}s.`,
          );
          ...
          this.logger.error(
            `[REDIS CACHE] Error saving to Redis: ${err.message}`,
          );
        }
      }
    }
  ...
  private async forwardTelemetry(
    ...
    try {
      ...
      this.logger.error(
        `[HOOK] onTelemetry failed for ${deviceId}: ${err.message}`,
      );
  ...
  private normalizeTelemetry(
    ...
    try {
      ...
      this.logger.error(
        `[NORMALIZATION] Mapping normalization failed for ${deviceId}: ${err.message}`,
      );
      throw new NormalizationFailedException();
    }
  }

  private findTelemetryField(
    mapping: any,
    statePath: string,
  ): string | null {
  ...
  private isCommandRedundant(
    ...
    this.logger.debug(
      `[REDUNDANCY] command=${command} current=${currentValue} requested=${requestedValue}`,
    );
  ...
  async processTelemetry(
    ...
    try {
      ...
      this.logger.debug(
        `[SUCCESS] Forwarding normalized data to host application via onTelemetry hook...`,
      );
      ...
      if (error instanceof ConfigMissingException) {
        this.logger.warn(
          `[CONFIG] Device ${deviceId} is missing configuration.`,
        );
      ...
      if (error instanceof NormalizationFailedException) {
        this.logger.warn(
          `[NORMALIZATION] Device ${deviceId} normalization failed.`,
        );
      ...
      if (error instanceof InvalidTimestampException) {
        this.logger.warn(
          `[TIMESTAMP] Device ${deviceId} sent invalid timestamp.`,
        );
        throw error;
      }
      ...
      this.logger.error(
        `[UNHANDLED] Unexpected error for device ${deviceId}: ${error.message}`,
      );
  ...
  async processStatus(
    ...
    this.logger.log(
      `[STATUS LOG] Device: ${deviceId} changed state -> ${status.toUpperCase()}`,
    );
  ...
  async triggerDeviceTelemetry(
    ...
    this.logger.warn(`[TRIGGER] device=${deviceId} requestedState=${state}`);
    ...
    try {
      ...
      this.logger.warn(`Command failed for ${deviceId}: ${err.message}`);
      throw err;
    } finally {
  ...
  async executeCommand(
    ...
    console.log('LATEST TELEMETRY:', JSON.stringify(latest?.data, null, 2));
    ...
    if (command === 'SET_STATE') {
      return this.triggerDeviceTelemetry(deviceId, payload.state, context);
    }
  ...
  async getCommandMetadata(deviceId: string) {
    ...
    return Object.entries(commands).map(([commandName, commandDef]: any) => ({
      command: commandName,
